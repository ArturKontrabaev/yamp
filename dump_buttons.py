#!/usr/bin/env python3
"""Dump all buttons in Yandex Music player bar"""
...
(function() {
    ...
    if (!bar) return JSON.stringify(["NO PLAYER BAR FOUND"]);

    var all = bar.querySelectorAll('button, a, [role="button"], svg, [class*="ontrol"], [class*="ike"], [class*="avorit"]');
    for (var e of all) {
        var tag = e.tagName;
        var c = (typeof e.className === 'string') ? e.className : '';
        var t = e.title || '';
        var ar = e.getAttribute('aria-label') || '';
        var txt = e.textContent.trim().substring(0, 30);
        if (c || t || ar) {
            r.push(tag + ' | cls:' + c.substring(0, 70) + ' | title:' + t + ' | aria:' + ar + ' | text:' + txt);
        }
    }
